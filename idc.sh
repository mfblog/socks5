...
# ==============================================================================
# 安装脚本：自动创建idc流量监控服务 (V5 - 交互式阈值设置)
# ==============================================================================

# 检查是否以root用户身份运行
if [ "$EUID" -ne 0 ]; then
  echo "错误：此脚本必须以root用户身份运行。"
  echo "请尝试使用 'sudo ./idc.sh' 来执行。"
  exit 1
fi

# --- 【V5 新增】用户交互：获取流量阈值 ---
USER_THRESHOLD_GB=""
while ! [[ "$USER_THRESHOLD_GB" =~ ^[1-9][0-9]*$ ]]; do
    read -p "请输入每月总流量阈值 (单位: GB，例如: 1024): " USER_THRESHOLD_GB
    if ! [[ "$USER_THRESHOLD_GB" =~ ^[1-9][0-9]*$ ]]; then
        echo "错误：请输入一个有效的正整数。"
    fi
done
echo "✅ 流量阈值已设置为: ${USER_THRESHOLD_GB} GB"
echo ""
# --- 交互结束 ---
...
# --- 核心流量监控脚本内容 ---
# 注意：下面的'EOF'没有引号，是为了让 ${USER_THRESHOLD_GB} 变量能被正确替换
...
CHAT_ID="2058570154"
# 【V5】此值由安装脚本动态写入
THRESHOLD_GB=${USER_THRESHOLD_GB}
CHECK_INTERVAL=10
...
send_notification() {
    local message="\$1"; echo "正在发送通知..."; local URL="https://api.telegram.org/bot\${BOT_TOKEN}/sendMessage"; local response; http_code=\$(curl -s -X POST "\$URL" -d chat_id="\${CHAT_ID}" -d text="\${message}" --connect-timeout 10 --max-time 15 -w "%{http_code}" -o >(response=\$(cat))); echo "Telegram API HTTP Status: \${http_code}"; echo "Telegram API Response Body: \${response}"; if [ "\$http_code" -eq 200 ]; then echo "通知已成功发送 (HTTP 200)。"; else echo "警告: 发送通知失败。HTTP状态码: \${http_code}"; fi
}

send_startup_info() {
    echo "正在准备并发送启动信息..."
    local MY_PUBLIC_IP; MY_PUBLIC_IP=\$(curl -s --connect-timeout 5 https://ipinfo.io/ip); if [ -z "\$MY_PUBLIC_IP" ]; then echo "警告: 无法获取公网IP地址，启动信息可能不完整。"; MY_PUBLIC_IP="获取失败"; fi
    local total_traffic_bytes; total_traffic_bytes=\$(cat /proc/net/dev | tail -n +3 | grep -v "lo:" | awk '{rx += \$2; tx += \$10} END {print rx+tx}'); local current_traffic; current_traffic=\$(format_bytes "\$total_traffic_bytes")
    local MESSAGE="🚀 服务启动通知%0A%0A主机: [\$(hostname)]%0A公网IP: \${MY_PUBLIC_IP}%0A当前总流量: \${current_traffic}%0A本月阈值: \${THRESHOLD_GB} GB (双向)"
...
while true; do
    ...
    if [ -f "\$STATE_FILE" ] || [ -f "\$OVERRIDE_FILE" ]; then if [ -f "\$STATE_FILE" ]; then echo "网络当前处于自动阻断状态，等待解锁命令或月初重置。"; fi; if [ -f "\$OVERRIDE_FILE" ]; then echo "网络当前处于手动覆盖状态，本月内不再自动检查流量。"; fi; else
        total_traffic_bytes=\$(cat /proc/net/dev | tail -n +3 | grep -v "lo:" | awk '{rx += \$2; tx += \$10} END {print rx+tx}')
        if (( total_traffic_bytes > THRESHOLD_BYTES )); then
            PUBLIC_IP=\$(curl -s https://ipinfo.io/ip); current_traffic=\$(format_bytes "\$total_traffic_bytes");
            MESSAGE="流量警告：主机 [\$(hostname)] 的总流量已超出阈值！即将阻断网络！%0A%0A阈值: \${THRESHOLD_GB} GB (双向)%0A当前总流量: \${current_traffic}%0A公网IP: \${PUBLIC_IP}%0A%0A你可以通过发送 '/unblock \${PUBLIC_IP}' 来手动解锁。%0A网络将在下月1号自动恢复。"
            send_notification "\$MESSAGE"; block_traffic_auto;
        else
            echo "当前总流量: \$(format_bytes "\$total_traffic_bytes") (阈值: \${THRESHOLD_GB} GB)，正常。"
        fi
    fi
